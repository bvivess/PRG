...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            for (Meteorit m : gestor.meteorits) {
                gestor.geoPositions.add(m.getGeoPosition());
            }
            
            // 1. > 1000g
            System.out.println("METEORITS > 1kg (v?lids):");
            System.out.println( gestor.meteorits.stream()
                                                .filter(m -> m.getMass() > 1000 && m.getType().equals("Valid") )
                                                .count() );

            // 2. per any
            System.out.println("\nCOUNT PER YEAR:");
            gestor.meteoritsPerAny = gestor.meteorits.stream()
                                                     .collect( Collectors.groupingBy(m -> m.getYear()) );
            gestor.meteoritsPerAny.entrySet().stream()
            ...
            // 3. més pesat
            Meteorit max = gestor.meteorits.stream()
            ...
            // 4. caiguts en Europa
            System.out.println("\nCAIGUTS EN EUROPA: ");
            List<Meteorit> europeus = gestor.meteorits.stream()
            ...
            System.out.println("\nCAIGUTS EN ESPANYA: ");
            List<Meteorit> mallorquins = gestor.meteorits.stream()
